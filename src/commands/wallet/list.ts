...
import {isEncryptedWallet} from '../../lib/store.js'
  ...
  async run(): Promise<void> {
    ...
    if (flags.json) {
      const list = names.map((name) => ({
        ...
        isEncrypted: isEncryptedWallet(data.wallets[name]!),
        name,
      }))
    ...
    for (const name of names) {
      ...
      const defaultMark = config.default === name ? ' ' + style.info('(default)') : ''
      const lockMark = isEncryptedWallet(w) ? ' ' + style.dim('(encrypted)') : ''
      this.log(
        fancy
          ? style.label(name) + style.dim(': ') + style.value(w.address) + defaultMark + lockMark
          : `${name}: ${w.address}${config.default === name ? ' (default)' : ''}${isEncryptedWallet(w) ? ' (encrypted)' : ''}`
      )
    }
  }
}
